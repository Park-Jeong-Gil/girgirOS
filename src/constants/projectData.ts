...
<div class="siteDesc">
  <p><strong>Cosmofy</strong>는 cosmos(우주) + comfy(편안함)의 합성어로 온전하게 개인만을 위한 프라이빗 SNS 입니다.</p>
  <p>현재의 SNS는 사용자들에게 소통의 즐거움을 주는 반면, 피로감도 동반 할 수 밖에 없다고 생각 합니다. 사적인 기록마저 익명의 불특정 다수에게 노출 되어야 하는 구조니까요.</p>
  <p>저는 이 부분이 SNS의 명암이라 생각했습니다.</p><p>그래서 반대로 비공개를 디폴트로 경우에 따라 제한적인 공유와 소통이 가능한, 새로운 SNS도 필요하다는 생각에서 <strong>Cosmofy</strong>의 개발을 시작했습니다.</p>
  <p>아직은 서비스라 부르기도 민망한 1인 프로젝트이지만, 언젠가 저 처럼 현재의 SNS에 피로감을 느끼는 사람들에게 편안함을 줄 수 있는 정식 SNS가 되길 바라며, 꾸준히 업데이트 해보겠습니다.</p>
</div>
    `,
  },
  KG: {
  ...
  BROADCAST: {
    ...
    DESCRIPTION: `
      ...
      <div class="siteDesc">
        <p>방송통신 위원회의 전자 심의 시스템을 PC, Mobile 버전으로 개발 했습니다.</p>
        <p>디자인과 프론트 개발을 모두 진행 했으며, 복잡한 데이터 구조를 효율적으로 처리할 수 있는 UI/UX를 구현했습니다.</p>
      </div>
    `,
  },
  ENUMCUT: {
